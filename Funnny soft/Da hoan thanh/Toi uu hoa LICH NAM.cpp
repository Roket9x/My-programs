...
#include <stdio.h>
#include <conio.h>
unsigned char sntt(unsigned char m, unsigned char y)
{
...
unsigned char  moc1(unsigned char d, unsigned char m,unsigned int y)
{
	unsigned int i,dd;// dd khoang cach ngay thang nam 1/1/1-->d/m/1
	// ti nua cong them (y-1) nam
	dd=d-1;
	for (i=1; i<=m-1; i++)
	{
...
long moc2(unsigned int y)// tinh khoang cach ngay tu ngay 1/1/2018--> 1/1/y 
{
	unsigned long dd=0;
	unsigned int i=1;
	while (i<y)
...
int main()
{
	...
	for (int i=1; i<=12; i++)// lap thang
	{
		 int j,l;
		 printf("Thang %d: \n",i); 
		 for (l=2; l<=7; l++) printf("T%d ",l); // in thu:
		  printf("CN ");
		 printf("\n");
		 printf("--------------------\n");
