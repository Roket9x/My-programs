...
void transmit(short td_snake[2][long_max], short ddai, char &dk, char not_dk)// dk: nhan lenh dieu khien
{
	...
	bool tu_chet(short td_snake[2][long_max], short ddai)
	{
		...
		&& td_snake[0][ddai-1]==td_snake[0][i]) 
		{
			printf("%d %d\n",td_snake[0][i],td_snake[1][i]);
			return true;
...
int main()
{
	...
	sinh_moi(x_food, y_food, td_snake, ddai, ok_sinh_moi);	
		...
		{
				dk= getch();
				if (dk*not_dk=='a'*'d') dk=not_dk;
				if (dk*not_dk=='w'*'s') dk=not_dk;
				...
				if (stop) 
				{
					//printf("%c %c %d %d %d %d",dk,not_dk,td_snake[0][ddai-1],td_snake[1][ddai-1],td_snake[0][ddai-2],td_snake[1][ddai-2]);
					printf("%c %c",not_dk,dk );
					in_td_snake(td_snake,ddai);
					printf("\n%d--%d-%s",sl,strlen(sdk)/2+1,sdk);
				}
				if (dk=='w' || dk=='s' || dk=='a' || dk=='d')
