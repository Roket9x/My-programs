...
*/
// Da Nang cap: toc do tang theo do dai==> toi uu lai
#include <stdio.h>
#include <conio.h>
#include <time.h>
#include <stdlib.h>
	...
	sinh_moi(x_food, y_food, td_snake, ddai, ok_sinh_moi);	
		while (1)
		{
				...
				while (!kbhit() && !stop) 
				{
					...
					display(ddai,td_snake);
					if (lv*50-stop>20) stop=ddai-3;; else stop=lv*50-30;
					if (dk=='w' || dk=='s') delay(lv*90-stop); else delay(lv*50-stop);
					if (ok_sinh_moi) 
			...
			if (stop) { end(ddai, d_khung, r_khung); break; }
